...
class Mergeable {
  ...
  flex (robot) {
    ...
    robot.on('pull_request', handler);
    robot.on('pull_request_review', handler);
    robot.on('pull_request_review_comment', handler);
    robot.on('issues', handler);
  }
}
